...
def distAng (plot):
    ...
    for i in range( len(i0Particuliers) -1):
        ...
        x, y = sim.trajetRayonEuler(c.Xinterval, [c.y0[0], i0Particuliers[i] ], c.dx, sim.profilTemperatureLin)
        angleArrivée[i] = y[1][-1]
    
    print('angles des rayons en xf: ', angleArrivée)
    distAngu = abs( angleArrivée[0] - angleArrivée[1])
    print("distance angulaire entre les rayons en xf : ", round(distAngu*1e3, 6), " *10^-3 rad")
...
def imagesVoiture():
    ...
    # faire fct qui définit un z pour chaque pixel (ex : l'image fait 2m, on divise par le nb de pixel et chaque pixel = k * 2/nb pix) 
